"""
Pix2Pix + Perceptual Loss 训练脚本（E7: L1 + GAN + Perceptual）。

对应 Guide.md:
- 7.1: U-Net 生成器 + PatchGAN 判别器
- 7.2.1: 基础损失（L1 + GAN）
- 7.2.2: 扩展损失（Perceptual Loss）
- 7.3: 训练策略（Adam, lr=2e-4, 200 epochs, batch size=1, 线性衰减）
- 11.1 E7: Perceptual Loss 尝试
"""
...
def train_epoch(
    ...
    percep_criterion: PerceptualLoss,
    dataloader,
    optimizer_G,
    optimizer_D,
    device,
    ...
    """训练一个 epoch：交替更新 D 和 G（带 Perceptual Loss）。"""
    generator.train()
    discriminator.train()
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [Train Pix2Pix+Perceptual]")
    for batch_idx, batch in enumerate(pbar):
        label = batch["label"].to(device)
        photo = batch["photo"].to(device)

        # -------------------------
        # 1. 更新判别器 D（与 E2 相同，只用 GAN 损失）
        # -------------------------
        ...
        # Perceptual Loss：对 fake_photo 和 photo 做 VGG 特征距离
        g_perc = percep_criterion(fake_photo, photo)

        g_total = g_total_basic + lambda_perc * g_perc
        g_total.backward()
        optimizer_G.step()
        ...
        if writer is not None and batch_idx % 100 == 0:
            ...
            writer.add_scalar("Train/G_perc", g_perc.item(), global_step)
    ...
    avg_g_perc = total_g_perc / num_batches
    avg_d = total_d / num_batches
    return avg_g_total, avg_g_gan, avg_g_l1, avg_g_perc, avg_d
...
def validate(
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [Val Pix2Pix+Perceptual]")
    for batch_idx, batch in enumerate(pbar):
        label = batch["label"].to(device)
        photo = batch["photo"].to(device)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Pix2Pix with Perceptual Loss (E7: L1 + GAN + Perceptual)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exp-name",
        type=str,
        default="pix2pix_l1_gan_perc_strong",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Weight for Perceptual loss in generator",
    )

    # 数据增强配置（与 E2/E6 一致，默认 strong）
    parser.add_argument(
        "--aug-mode",
        type=str,
        default="strong",
    ...
    # 数据增强配置（与 train_unet / train_pix2pix / train_pix2pix_e6_fm 保持一致）
    aug_configs = {
    ...
    # 感知损失
    percep_criterion = PerceptualLoss().to(device)
    ...
    if args.resume is not None and args.resume.exists():
        ...
        # 恢复 history（如果存在）
        history_file = exp_dir / "logs" / "history_pix2pix_perc.json"
        ...
        print(
            f"\n🚀 开始新的 Pix2Pix+Perceptual 训练 "
            f"(aug_mode={args.aug_mode}, lambda_l1={args.lambda_l1}, lambda_perc={args.lambda_perc})..."
        )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(
            ...
            f"G_perc: {train_g_perc:.4f}, D_loss: {train_d:.4f}, "
        ...
        history["train_g_perc"].append(train_g_perc)
        ...
        with open(
            exp_dir / "logs" / "history_pix2pix_perc.json", "w", encoding="utf-8"
        ) as f:
    ...
    writer.close()
    print(
        f"\n✅ Pix2Pix+Perceptual training complete! "
