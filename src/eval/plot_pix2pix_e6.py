...
def load_history(exp_dir: Path):
    history_path = exp_dir / "logs" / "history_pix2pix_fm.json"
...
def plot_losses(history: dict, out_dir: Path):
    ...
    train_g_fm = np.array(history["train_g_fm"])
    ...
    plt.plot(epochs, train_g_fm, label="G Feature Matching (train)")
    plt.plot(epochs, train_d, label="D loss (train)")
    plt.plot(epochs, val_l1, label="L1 (val)", linestyle="--", linewidth=2)

    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Pix2Pix (L1 + GAN + Feature Matching) - Training / Validation Losses")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()

    out_path = out_dir / "e6_pix2pix_fm_losses.png"
...
def plot_metrics(history: dict, out_dir: Path):
    ...
    out_path = out_dir / "e6_pix2pix_fm_val_metrics.png"
...
def make_main_results_table(history: dict, out_dir: Path):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    table_path = out_dir / "e6_pix2pix_fm_main_results.md"
...
def collect_sample_images(exp_dir: Path, out_images_root: Path):
    ...
    dst_dir = out_images_root / "e6_pix2pix_l1_gan_fm_strong"
...
def main():
    exp_dir = PROJECT_ROOT / "outputs" / "pix2pix_l1_gan_fm_strong_e50"
    figures_dir = PROJECT_ROOT / "outputs" / "figures" / "pix2pix_l1_gan_fm_strong_e50"
    ...
    print("[OK] Pix2Pix E6 (Feature Matching, 50 epochs) analysis figures and table generated.")
    print(f"  - Loss & metrics figures: {figures_dir}")
    print(f"  - Sample images copied under: {images_root / 'e6_pix2pix_l1_gan_fm_strong'}")
